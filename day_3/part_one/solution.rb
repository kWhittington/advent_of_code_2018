...
# Point => Claim Count
claimed_points = Hash.new(0)

input_pathname.each_line do |line|
  claim = Claim.from_string(line.chomp)
  (claim.left...claim.right).each do |x|
    (claim.top...claim.bottom).each do |y|
      claimed_points[[x, y]] += 1
    end
  end
end

puts "Claimed Points: #{claimed_points.keys.uniq.length}"

shared_points = claimed_points.select { |_, v| v > 1 }

puts "Shared Points: #{shared_points.keys.uniq.length}"
# Outputs:
# Claimed Points: 333112
# Shared Points: 110891
#
# Answer: 110891
